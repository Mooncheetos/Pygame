...
IMAGE_PATH = "Images/Goose for game"
...
def create_enemy():
    enemy_size = (30, 30)
    enemy = pygame.image.load('Images/enemy.png').convert_alpha() #pygame.Surface(enemy_size)
    # enemy.fill(COLOR_BLUE)
    enemy_rect = pygame.Rect((WIDTH + random.randint(10, 100)), random.randint(0, HEIGHT), *enemy_size)
    enemy_move = [random.randint(-8, -4), 0]
    return [enemy, enemy_rect, enemy_move]

def create_bonus():
    bonus_size = (23, 23)
    bonus = pygame.image.load('Images/bonus.png').convert_alpha() # pygame.Surface(bonus_size)
    # bonus.fill(COLOR_BONUS)
    bonus_rect = pygame.Rect(random.randint(0, WIDTH), random.randint(0, HEIGHT // 4), *bonus_size)
